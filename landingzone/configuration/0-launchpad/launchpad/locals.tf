...
locals {
  ...
  config = (local.config_file_extension == local.const_yaml ?
    yamldecode(templatefile("./../../../../../config/${local.config_file_name}", local.config_template_file_variables)) :
    jsondecode(templatefile("./../../../../../config/${local.config_file_name}", local.config_template_file_variables))
  )
}
